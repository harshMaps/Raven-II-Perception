...
in terminal, run:
python3 examples/automated_IBIS_6.py splash --weights=/media/yq/0d57f6b9-4507-4edb-9800-537ac5258c2b/home/hdd_files/samin/dsn2019/prelimstudy/Suturing/logs/surgery20191111T2135/mask_rcnn_surgery_0020.h5
...
2/6/2020:
Updated ZED SDK and transfered to Server 2
cd to:
~/Documents/Kay/Devon/zed-python-api-legacy
run:
python3 examples/automated_IBIS_6.py splash --weights=/home/student/Documents/Kay/Devon/zed-python-api-legacy/surgery20191111T2135/mask_rcnn_surgery_0020.h5
...
#camera_settings = sl.PyCAMERA_SETTINGS_BRIGHTNESS
...
class_names = ['Background', 'Right Grasper', 'Left Grasper', 'Red Block', 'Green Block']
############################################################
#  Configurations
############################################################
...
def txt_to_csv(position):
    ...
    with open('/home/student/Documents/Kay/Devon/zed-python-api-legacy/examples/log.csv', 'w') as out_file:
...
# pair lists
def pairlists(leftlist, rightlist):
        ...
        # The maximum number of objects that could have pairs is the number of objects in the shortest list
        for i in range(min(leftlen, rightlen)):
            testshort = shortlist[i]

            # for each unpaired object in the longlist, check if it matches the object from the shortlist
            # if it does, add it to the paired lists and remove the object from the longlist
            # if no object from the longlist matches the object from the shortlist, assume there is no pair
            # and move on to the next object in the short list
            for j in range(len(longlist)):
                testlong = longlist[j]
                # if the difference in y pixel coordinate is less than 15, it's a pair
                if abs(testshort[1][1] - testlong[1][1]) < 15:
                    pairedlistshort.append(testshort)
                    pairedlistlong.append(testlong)
                    longlist.pop(j)
                    break
                else:
                    pass

        # return left list first, then right list
        if rightlen < leftlen:
            return pairedlistlong, pairedlistshort
        else:
            return pairedlistshort, pairedlistlong
...
def project_to_cplane_and_control(pos, object_centers, centroid):
    ...
    centuvdist = np.sqrt((centuv[0]**2)+(centuv[1]**2))

    # Calculate the max size of the visible plane with the centroid
    # (middle to top of rectangle) (tan(60 deg) = 1.732, tan(30 deg) = 0.57735)
    vertvis = 0.57735 * dcam
    ...
    #if ((abs(centuv[1] - 0)) > 2*threshold) and (centuvdist > 3*threshold):
    if centuvdist > 3*threshold:
        if (centuv[1] > 2*threshold):
            # Up
            #ser.write(b'U')
            print("Command: Up")
        if (centuv[1] < -2*threshold):
    ...
    # Control L/R for pan
    #if ((abs(centuv[0] - 0)) > 2*threshold) and (centuvdist > 3*threshold):
    if centuvdist > 3*threshold:
        if (centuv[0] > 2*threshold):
            # Right
            #ser.write(b'R')
            print("Command: Right")
        if (centuv[0] < -2*threshold):
...
if __name__ == '__main__':
    ...
    if status != sl.ERROR_CODE.SUCCESS:    #tp.PyERROR_CODE.PySUCCESS:
        print(repr(status))
        exit()
...
# ----------------------------------------------------------------------------------------------
    ...
    while key != 113:   # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            #print(type(mat))
            #cv2.imshow("ZED Left", mat.get_data())
            cv2.imwrite("/home/student/Documents/Kay/Devon/zed-python-api-legacy/test_02122020/test_img.jpg", mat.get_data())
            img_left = mat.get_data()[:,:,0:3]
            #img_left = cv2.imread("/home/student/Documents/Kay/Devon/zed-python-api-legacy/test_02122020/test_img.jpg")
            ...
            #cv2.imshow("ZED Right", mat.get_data())
    ...
    # ---------------------------------------------------------------------------------------------------------------------
            ...
            if len(results_list_left) > 0:
                print("from ZED")
                left_image_object_list = results_list_left
                right_image_object_list = results_list_right
                #print("sorted: ")
            ...
            try:
                ...
                IBISposition = [54607.0, 1.57, 2.84, 2.53, 0.99, -0.11]
                ...
                sortedlistright = sort_objects(right_image_object_centers)
                sortedlistleft = sort_objects(left_image_object_centers)
                print("sorted lists:")
                print(sortedlistleft)
                print(sortedlistright)

                print("paired lists:")
                leftpairedlist, rightpairedlist = pairlists(sortedlistleft, sortedlistright)
                print(leftpairedlist)
                print(rightpairedlist)
                ...
                y_tot = 0
                z_tot = 0
                n = len(leftpairedlist)
                        #metavar="path or URL to video",

                # Use paired lists to calculate object positions
                for object in range(len(leftpairedlist)):
                    # create a list for each object to contain the object and the coordinates of its center
                    obj = [leftpairedlist[object][0]]
                    obj_pos = object_position(IBISposition, leftpairedlist[object][1], rightpairedlist[object][1])
                    #print(type(obj_pos))
                    #print(obj_pos)
                ...
                #print('\n')
                print("object centers from IBIS")
